...
Cayde.on("message", message => {
    ...
    if(cmd === `${prefix}help`)
        {
            ...
            .addField("kick (@user) (reason):", "Easily kick a user from the server! Note: reason is required. `A minimum of the 'KICK_MEMBERS' permission is required.`")
            .addField("ban (@user) (reason):", "Easily ban a user from the server! Note: reason is required. `A minimum of the 'BAN_MEMBERS' permission is required.`")
        ...
        {"thumbnail": "https://vignette.wikia.nocookie.net/destinypedia/images/2/24/Cayde-6.png/revision/latest?cb=20140622223202"}, false
      ));
        }
    ...
    message.channel.send(message_content);
    ...
    else if(cmd === `${prefix}kick`)
        {
          ...
          kickedUser.send("You got warned three times, and you were kicked. Next time is a ban. **Watch your footing.**")
          .catch(() => message.channel.send("Kick message could not be sent. And that makes me sad."))
          return;
        }
        ...
        else if(cmd === `${prefix}ban`)
        {
          ...
          .catch(() => message.channel.send("Ban message could not be sent. And that makes me sad."))
          return;
        }
    ...
    if(!userInfoToGet)
      return message.channel.send("Who do you want to get information for?");
  ...
  return rand[Math.floor(Math.random()*rand.length)];
          ...
          message.channel.send(eightBallAnswers());
      }
...
//Cayde.on("message", message => {
  //message.pin();
//})
//385 lines of pure code, FeelsGoodMan
